...
_Generated 2026-07-03 10:02:35_
...
| IF | 1.0000 | 0.9500 | ✓ | undef rate 18.18% |
...
- `http://example.org/Oscar2013` — **1** triple(s) in sample
    - `http://example.org/awardedTo → http://example.org/Tarantino`
- `http://example.org/Movie1` — **1** triple(s) in sample
    - `http://example.org/director → http://example.org/Tarantino`

### By predicate

Predicates ranked by number of source triples with no PGIU match:

- `http://example.org/awardedTo` — **1** triple(s)
    - `http://example.org/Oscar2013` → `http://example.org/Tarantino`
- `http://example.org/director` — **1** triple(s)
    - `http://example.org/Movie1` → `http://example.org/Tarantino`
...
- Elements without a μ_e match (or with no scorable kind set): **0**
