...
_Generated 2026-07-03 10:02:26_
...
- Derived triples (|derived_triples(C)|): **8**
...
|---|---|---|---|---|
| IP | 0.8000 | 0.9500 | ✗ | 4/5 full |
| IF | 1.0000 | 0.9500 | ✓ | undef rate 18.18% |
| IR | 1.0000 | 0.9500 | ✓ | universe=7 |
...
- `http://example.org/Movie1` — **1** triple(s) in sample
    - `http://www.w3.org/1999/02/22-rdf-syntax-ns#type → http://example.org/Movie`

### By predicate

Predicates ranked by number of source triples with no PGIU match:

- `http://www.w3.org/1999/02/22-rdf-syntax-ns#type` — **1** triple(s)
    - `http://example.org/Movie1` → `http://example.org/Movie`
...
- Elements without a μ_e match (or with no scorable kind set): **1**
- Occurrences with no covering PGIU: **1**
...
- full: **7** ; partial: **0** ; local: **0** / universe 7
